...
def renderView(request):
    ...
    if requestOptions['graphType'] == 'pie':
        ...
        if format == 'json':
            ...
            if 'maxDataPoints' in requestOptions and any(data):
                ...
                for series in data:
                    ...
                    if maxDataPoints < numberOfDataPoints:
                        ...
                        nudge = secondsPerPoint + (series.start % series.step
                                                   ) - (series.start %
                                                        secondsPerPoint)
                        ...
                        timestamps = range(
                            int(series.start),
                            int(series.end) + 1, int(secondsPerPoint))
                    else:
                        timestamps = range(
                            int(series.start),
                    ...
                    series_data.append(
                        dict(
                            target=series.name, datapoints=datapoints))
            else:
                for series in data:
                    timestamps = range(
                        int(series.start),
            ...
            if 'jsonp' in requestOptions:
                ...
                response = HttpResponse(
                    content=json.dumps(series_data),
        ...
        if format == 'raw':
            ...
            for series in data:
                response.write("%s,%d,%d,%d|" % (series.name, series.start,
    ...
    if useSVG and 'jsonp' in requestOptions:
        ...
        response = buildResponse(image, 'image/svg+xml'
                                 if useSVG else 'image/png')
...
def parseOptions(request):
    ...
    for opt in graphClass.customizable:
        if opt in queryParams:
            ...
            if (val.isdigit() or
                (val.startswith('-') and
                 val[1:].isdigit())) and 'color' not in opt.lower():
                val = int(val)
            elif '.' in val and (val.replace('.', '', 1).isdigit() or
                                 (val.startswith('-') and
                                  val[1:].replace('.', '', 1).isdigit())):
                val = float(val)
